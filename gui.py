...
class RepoExtractorGUI:
    ...
    def _create_ui(self):
        ...
        repo_frame = ttk.LabelFrame(
            main_container, text="Repository Selection", padding=10
        )
        repo_frame.grid(row=current_row, column=0, sticky="ew", pady=(0, 10))
        repo_frame.columnconfigure(1, weight=1)

        self.repo_path_var = tk.StringVar()
        ttk.Label(repo_frame, text="Repository Path:").grid(
            row=0, column=0, sticky="w", padx=(0, 5), pady=5
        )
        ttk.Entry(repo_frame, textvariable=self.repo_path_var, width=60).grid(
            row=0, column=1, sticky="ew", padx=5, pady=5
        )

        repo_btn_frame = ttk.Frame(repo_frame)
        repo_btn_frame.grid(row=0, column=2, sticky="e", padx=(5, 0), pady=5)
        ttk.Button(repo_btn_frame, text="Browse...", command=self._browse_repo).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(
            repo_btn_frame, text="Scan Repository", command=self._scan_repo
        ).pack(side=tk.LEFT)

        current_row += 1

        folder_frame = ttk.LabelFrame(
            main_container, text="Folder Exclusion", padding=10
        )
        folder_frame.grid(row=current_row, column=0, sticky="ew", pady=(0, 10))
        folder_frame.columnconfigure(1, weight=1)

        ttk.Label(folder_frame, text="Exclude Folders:").grid(
            row=0, column=0, sticky="w", padx=(0, 5), pady=5
        )
        self.exclude_folders_var = tk.StringVar()
        ttk.Entry(folder_frame, textvariable=self.exclude_folders_var, width=50).grid(
            row=0, column=1, sticky="ew", padx=5, pady=5
        )
        ttk.Label(folder_frame, text="(comma separated)").grid(
            row=0, column=2, sticky="w", padx=(0, 5), pady=5
        )
        ttk.Button(
            folder_frame, text="Apply Exclusions", command=self._apply_folder_filters
        ).grid(row=0, column=3, padx=(5, 0), pady=5)

        current_row += 1

        files_frame = ttk.LabelFrame(
            main_container, text="Select Files to Include", padding=10
        )
        ...
        ttk.Button(
            btn_frame, text="Toggle Selected", command=self._toggle_selected
        ).pack(side=tk.LEFT)
        ...
        ttk.Button(filter_btn_frame, text="Apply", command=self._apply_filter).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(filter_btn_frame, text="Clear", command=self._clear_filter).pack(
            side=tk.LEFT
        )
        ...
        ttk.Label(output_config_frame, text="Output Format:").grid(
            row=0, column=0, sticky="w", padx=(0, 5), pady=5
        )
        ...
        output_frame = ttk.LabelFrame(
            main_container, text="Output Selection", padding=10
        )
        ...
        ttk.Entry(output_frame, textvariable=self.output_path_var, width=60).grid(
            row=0, column=1, sticky="ew", padx=5, pady=5
        )
    ...
    def _update_output_extension(self, event=None):
        ...
        new_ext = (
            ...
            else ".json" if format_type == "json" else ".txt"
        )
    ...
    def _browse_repo(self):
        ...
        if repo_path:
            ...
            extension = (
                ...
                else ".json" if output_format == "json" else ".txt"
            )
            ...
            self.output_path_var.set(
                os.path.join(compendium_dir, f"{repo_name}_compendium{extension}")
            )
    ...
    def _browse_output(self):
        ...
        output_path = filedialog.asksaveasfilename(
            ...
            defaultextension=(
                ...
                else ".json" if selected_format == "json" else ".txt"
            ),
        )
    ...
    def _scan_repo(self):
        ...
        if self.exclude_folders_var.get().strip():
            excluded_folders = {
                folder.strip()
                for folder in self.exclude_folders_var.get().split(",")
                if folder.strip()
            }
        ...
        try:
            self.files_data = self.repo_extractor.scan_repository(
                repo_path, excluded_folders=excluded_folders
            )
            self._update_tree()
    ...
    def _apply_folder_filters(self):
        ...
        try:
            self.files_data = self.repo_extractor.scan_repository(
                self.repo_path, excluded_folders=excluded_folders
            )
            self._update_tree()

            status_msg = (
                f"Applied exclusions: Found {len(self.files_data)} files in repository."
            )
            if excluded_folders:
                status_msg += f" (Excluded {len(excluded_folders)} folders)"
            ...
            messagebox.showerror(
                "Error", f"Failed to apply folder exclusions: {str(e)}"
            )
            self.status_var.set("Ready")
    ...
    def _toggle_item(self, event):
        ...
        for file_info in self.files_data:
            if file_info["path"] == path:
                ...
                new_values = (path, values[1], "✓" if file_info["selected"] else "✗")
    ...
    def _generate_output(self):
        ...
        try:
            contents = self.repo_extractor.extract_files_content(
                repo_path, selected_files
            )
            ...
            self.status_var.set(
                f"Compendium generated successfully with {len(contents)} files."
            )
            messagebox.showinfo(
                "Success", f"Compendium generated successfully at {output_path}"
            )
        except Exception as e:
